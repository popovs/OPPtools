...
if(is.na(dat$breeding_pairs[i])) popSize <- NULL else popSize <- dat$breeding_pairs[i]*2

study_data <- move::getMovebankStudy(study = project, login = login)
dep_data <- move::getMovebankReferenceTable(study = project, login = login)
...
my_area_50 <- track2KBA::findSite(my_ud, represent = ra50$out, 
                                     popSize = popSize,
                                     levelUD = 50, polyOut = T, thresh = 10)

my_area_95 <- track2KBA::findSite(my_ud, represent = ra95$out, 
                                     popSize = popSize,
                                     levelUD = 95, polyOut = T, thresh = 10)

```

Kernel density estimates were calculated for each individual track using the median *'href'* smoothing parameter, `r h` km. Track representativeness was calculated using the track2KBA package. For core use areas, with utilization distribution of 50%, inclusivity was `r signif(ra50$out, 3)`% At the home range level, with utilization distribution of 95%, inclusivity was `r signif(ra95$out, 3)`%.

```{r map_kernel_95, echo=FALSE, message=FALSE, warning=FALSE, fig.cap= "Estimated number of birds with a home range (95% utilization distribution) that predictably overlaps areas around the (orange point)."}

map95 <- opp_map_keyareas(track2KBA_UD = my_area_95, 
...
```{r map_kernel_50, echo=FALSE, message=FALSE, warning=FALSE, fig.cap= "Estimated number of birds with a core range (50% utilization distribution) that predictably overlaps areas around the breeding colony (orange point)."}

map50 <- opp_map_keyareas(track2KBA_UD = my_area_50, 
                         center = my_track2kba$site, 
                         show_site = T, 
                         zoom = 6,
                         coast_scale = 10,
