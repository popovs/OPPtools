...
if (params$stage == 'Incubation') {
  ...
  stage_message <- 'This report focuses on tracking data collected during the incubation stage of the breeding season (Figure 2).'
}

if (params$stage == 'Chick-rearing') {
  dat <- dat %>% filter(animal_reproductive_condition %in% c('breeding, chicks', 'breeding, chick'))
  stage_message <- 'This report focuses on tracking data collected during the chick-rearing stage of the breeding season (Figure 2).'
}

if (params$stage == 'Unknown') {
    stage_message <- 'Breeding status could not be determined for all deployments, therefore data in this report represent both incubation and chick-rearing breeding stages (Figure 2).'
}

# Save raw Movebank data as shp
if (params$save_pts == TRUE) {
  pts <- dat
  pts <- sf::st_as_sf(dat, coords = c("location_long", "location_lat"), crs = 4326)
  pts$timestamp <- as.character(pts$timestamp)
  sf::st_write(pts, 
               paste0(file.path(here::here(), params$output_dir, params$file_name), " raw GPS pts.shp"), 
               append = FALSE,
               quiet = TRUE)
  rm(pts)
}
...
metadata <- list("smoother" = params$kernelSmoother, 
                 ...
                 "meta_lh" = "Life history metadata | species = species; site = colony/tagging site; stage = tagged animal life stage; col_pairs = total # of breeding pairs at colony (i.e. total colony population / 2)"
                 )
...
if (params$saveShp == TRUE) {
  sf::st_write(sites_out, 
               paste0(file.path(here::here(), params$output_dir, params$file_name), " OPP_Areas.shp"), 
               append = FALSE,
               quiet = TRUE)
}
