...
if (kernelSmoother == "href") {
  smooth = h
} else if (kernelSmoother == "href/2") {
  smooth = h/2
} else if (kernelSmoother == "step") {
  smooth = s
}

# Calc kernels
if (kernelSmoother != "bbmm") {
...
The `r params$kernelSmoother` smoother was chosen for kernel density estimation, with a value of `r if(kernelSmoother == "href"){ paste(h) } else if(kernelSmoother == "href/2") {paste(h/2)} else if(kernelSmoother == "step") {paste(s)} else {paste("Error: kernelSmoother not a valid value")}` km.

`r if(s == 1){"The step function resulted in a step smoother value of less than 1 km; therefore, 'step' kernel density estimates were calculated with a smoother value of 1 km."}`
