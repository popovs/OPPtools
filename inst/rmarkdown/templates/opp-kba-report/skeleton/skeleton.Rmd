...
bad_asymp <- (abs(ra_result$est_asym - ra_result$est_asym) > 0.1 )
# Check if criteria for representativeness are met
not_rep <- ifelse(small_sample == T | bad_asymp == T, T, F)
...
`r if (small_sample) "Due to the low sample size of tracks available for this population, we recommend collecting additional tracking data before trying to infer key at-sea area use during the breeding season for this population."`

`r if (small_sample == F & bad_asymp == T) "The estimated asymptote for the inclusion rate differed from the target asymptote, this is an indication that the tracking sample size is not sufficient to represent the source population, we recommend collecting additional tracking data before trying to infer key at-sea area use during the breeding season for this population."`
