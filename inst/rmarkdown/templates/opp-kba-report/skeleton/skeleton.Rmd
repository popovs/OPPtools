...
metadata <- list("smoother" = params$kernelSmoother, 
                 ...
                 'stage' = params$stage,
                 'tot_pairs' = params$breeding_pairs,
                 'meta' = "percentl = percentile of population encompassed in polygon; n_trcks = number of GPS tracks used to generated kernel distributions; perc_pop = percent of the colony population estimated to be encompassed within the polygon; n_indiv = number of individuals (population) estimated to be encompassed within the polygon; repr = representativeness (%) of the kernels compared to the total population's space use, as estimated by repAssess; total_ss = total number of tracks used to generate the kernels and subsequent polygon output; smoother = method used for smoothing tracked kernel distributions; smooth_km = value in kms of the smoother; ud_level = utilization distribution level chosen for kernels; smpl_yr = number of years of tracks; species = species; site = home colony site; lif_stf = life stage of tagged animals"
                 )

sites_out <- opp_sites(kernels,population = params$breeding_pairs * 2,
...
if (params$saveShp == TRUE) {
  sf::st_write(sites_out[!is.na(sites_out$percentile), ], 
...
```{r plot KBA, fig.height=5, fig.cap = "Map showing the estimated distribution of birds at sea around the breeding colony. Polygon shading indicates the absolute number of birds from the source population estimated to use that area. Polygon oultine indicates the estimated percent of home ranges that overlap the area."}
