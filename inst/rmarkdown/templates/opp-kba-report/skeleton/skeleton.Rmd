...
metadata <- list("ud_smooth_type" = params$kernelSmoother, 
                 "ud_smooth_km" = smooth,
                 "ud_level" = params$levelUD,
                 "sample_years" = length(yy),
                 "species" = params$spp,
                 "site" = unique(dat$study_site),
                 'life_stage' = params$stage
                 )

sites_opp <- opp_sites(kernels,population = params$breeding_pairs * 2,
...
if (params$saveShp == TRUE) {
  sf::st_write(sites_opp[!is.na(sites_opp$percentile), ], 
